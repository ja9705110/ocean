...
export function HostConsole({ code }: HostConsoleProps) {
  ...
  const refreshSnapshot = useCallback(async (eventId: string) => {
    setSnapshot(await fetchEventSnapshot(eventId));
  }, []);
  ...
  }, [refreshEvent, refreshParticipants, refreshDrawState, refreshSnapshot]);
