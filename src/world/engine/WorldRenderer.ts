...
const DRAIN_INTERVAL_MS: Record<AddMode, number> = {
  ...
  /*
    ...
    河速大約是每秒 67 像素，300 毫秒只走 20 像素，而簽名有 120 像素寬——
    報到時大家接連掃碼，連續幾個就黏成一團。上游那一小塊再怎麼挑位置
    也塞不下那麼多人，所以除了換位置，也要拉開時間。

    650 毫秒走 43 像素，配合上游的候選位置，連續進場的人就分得開了。
    這個間隔只在「同時有好幾個人剛上傳」時才感覺得到；
    一個人上傳的等待時間完全不受影響。
  */
  entrance: 650,
};
