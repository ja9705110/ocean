...
export interface WorldTemplate {
   ...
   * 角色要不要被強制留在畫面內。預設要。
   *
   * 海洋那種自由漫遊一定要夾——行為來不及反應時角色會游出去不回來。
   * 但河流不一樣：角色是沿著一條固定的路徑跑的，而那條路徑的頭尾
   * 本來就在畫面外。夾住的結果是簽名滑到邊緣就卡住不動，
   * 一整排疊在角落等著淡出，看起來就是「流動很生硬」。
   */
  readonly clampToBounds?: boolean;
  /**
   * 環境裝飾要不要畫在角色底下。預設畫在上面。
   *
   * 海洋的泡泡疊在角色前面才有前後層次。河流相反：那些光粒很亮，
   * 蓋在簽名上會讓名字讀不出來——而簽名就是這一頁的主角。
   */
  readonly ambientBelowCharacters?: boolean;
}
