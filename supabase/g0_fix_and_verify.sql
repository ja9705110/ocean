-- G0 修復與驗證腳本
--
-- 用途：重建遊戲房間所需的全部資料表與函式、重載 PostgREST 結構快取，
-- 並在最後印出一張表，直接告訴你每一項是否真的建立成功。
--
-- 可重複執行，不會刪除任何既有資料。
-- 請整份一次貼上執行，不要只跑其中一段。
...
-- ============================================================
-- 驗證：下面兩張表就是結果，全部都必須顯示「已建立」
-- ============================================================
with expected(obj) as (
  values ('game_sessions'), ('teams'), ('game_players'), ('team_results')
)
select
  e.obj as 資料表,
...
with expected(fn) as (
  values ('create_game_session'), ('join_game'), ('list_session_teams'),
         ...
         ('generate_team_code'), ('sync_team_player_count')
)
select
  e.fn as 函式名稱,
  case when p.oid is null then '缺少' else '已建立' end as 狀態,
  coalesce(pg_get_function_identity_arguments(p.oid), '-') as 參數
from expected e
left join pg_proc p
